...
public class MuteListCommand {
	...
	private static int listMutes(CommandSourceStack stack, int page) throws CommandSyntaxException {
		...
		stack.sendSuccess(FabricParser.fabric(border), false);
		if (!mutes.isEmpty()) {
			int a = page * 10;
			for (int i = (page - 1) * 10; a > i && size > i; i++) {
				...
				component.append(FabricChatModerator.playerName(mute));
				...
				stack.sendSuccess(FabricParser.fabric(component), false);
			}
		} else {
			stack.sendSuccess(FabricParser.fabric(new ChatTextComponent("No mutes")), false);
		}
		stack.sendSuccess(FabricParser.fabric(border), false);
		return size;
	}
